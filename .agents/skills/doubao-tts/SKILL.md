---
name: doubao-tts
description: Generate Mandarin and multilingual narration with Volcengine Doubao Speech 2.0. Use when creating Chinese voiceovers, when the user prefers Volcengine/Doubao TTS, or when narration needs character-level timestamp metadata for subtitles.
---
...
result = TTSSelector().execute({
    ...
    "text": "If AI really changes the future, how should ordinary people take part?",
...
result = DoubaoTTS().execute({
    "text": "Short sample listening text.",
    "voice_id": "zh_female_vv_uranus_bigtts",
...
6. Group captions by Mandarin semantic phrases before applying timestamps. Do not split only by fixed character count; it can break phrases like "without betting on a single company" or "may gradually be diluted" and hurt comprehension.
