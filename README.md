...
- **Agent-first production workflow** — your coding assistant reads pipeline manifests and stage director skills, then drives the production process step by step.
- **12+ video pipelines** — animated explainers, cinematic trailers, screen demos, talking-head edits, podcast repurposing, localization/dubbing, documentary montages, avatar videos, and more.
- **Reference-video analysis** — start from a YouTube Short, Reel, TikTok, or local clip and turn it into an original production plan with grounded pacing, structure, and style notes.
- **Real-footage path** — build videos from free/open stock and archival footage instead of relying only on AI-generated stills.
- **AI asset generation** — route to image, video, voice, music, subtitle, enhancement, and post-production tools through provider selectors.
- **Dual composition runtimes** — Remotion for React/data-driven scenes, HyperFrames for HTML/CSS/GSAP motion graphics, plus FFmpeg for post-production.
- **Quality gates and checkpoints** — schema validation, cost tracking, provider decision logs, ffprobe checks, frame sampling, audio analysis, and human approval at creative gates.
- **Free/local fallback options** — Piper TTS, FFmpeg, Remotion demos, stock/open media sources, and optional local GPU video models.
...
```bash
git clone https://github.com/minhhoit/OpenMontage.git
cd OpenMontage
make setup
make doctor
make run
...
make remotion-studio    # Open the Remotion Studio UI
make remotion-render    # Render the default Remotion composition
make test               # Run tests
```

OpenMontage is agent-driven rather than a long-running web server. Locally, `make setup` installs the runtimes, `make run` verifies the available providers, and your AI coding assistant runs the selected pipeline from the project files.

### AI Coding Assistant Model Guidance

OpenMontage does **not** call a general-purpose LLM API as a runtime orchestrator. The LLM is the coding assistant you run around this repository: Claude Code, Cursor, Copilot, Windsurf, Codex, or another agent that can read files and execute commands. The `config.yaml` file includes an `llm` block for future/adapter use and currently defaults to `provider: anthropic` with `model: null`, meaning "use the assistant/provider default."

Recommended choices as of **2026-06-30**:

| Priority | Suggested model/surface | Best for | Cost/quality note |
|----------|-------------------------|----------|-------------------|
| Best overall production quality | Claude Sonnet-class or latest GPT-5-class coding agent | Full OpenMontage pipeline work, stage planning, tool use, script/scene/edit decisions | Best balance for long multi-step creative engineering work. Use this as the default if cost is acceptable. |
| Best low-cost daily driver | Gemini Flash/Flash-Lite-class or GPT-5 thinking mini-class agent | Research, script drafts, simple scene plans, batch edits, README/docs updates | Strong price-performance; good for most iteration before final review. |
| Highest-quality review pass | Claude Opus-class, Gemini Pro-class, or latest GPT-5 full-reasoning model | Final script polish, complex debugging, architecture changes, high-stakes review | Use only for checkpoints or hard problems because cost is higher. |
| Lowest-cost experimentation | OpenRouter free/low-cost models or local Ollama/LM Studio models | Quick drafts, summarization, simple transformations | Cheap, but expect more supervision and stronger review gates. |

For this repo, a practical cost-optimized workflow is: draft with a low-cost model, run the pipeline and validations locally, then use a stronger model for the final proposal/script/scene-plan review before spending money on image, video, voice, or music providers.
...
Use this repository's GitHub Discussions to share work and ideas:
...
For bugs, feature requests, and workflow discussions, use this repository's GitHub Issues and GitHub Discussions so everything stays visible and actionable.
